...
// 1. AUTOCOMPLETADO DE PACIENTE POR RUT
function configurarFichaRapida() {
    ...
    inputRut.addEventListener('input', (e) => {
        // Filtrar caracteres que no sean dígitos o la letra K/k
        let valorLimpio = e.target.value.replace(/[^0-9kK]/g, '').toUpperCase();

        // Limitar a un máximo de 9 caracteres brutos (ej: 203759355)
        ...
        // Si solo se ha escrito 1 dígito
        ...
        // Agrupar con puntos de derecha a izquierda cada 3 dígitos
        ...
        // Devolver valor formateado visualmente en pantalla
        e.target.value = `${cuerpoFormateado}-${dv}`;
    });

    // CONSULTA AUTOMÁTICA AL SALIR DEL CAMPO (BLUR)
        ...
        fetch(`/sigas/modulos/ajax_buscar_paciente.php?rut=${encodeURIComponent(rut)}`)
            ...
            .then(data => {
                if (data && data.encontrado) {
                    ...
                    if (document.getElementById('folio') && p.folio) document.getElementById('folio').value = p.folio;
